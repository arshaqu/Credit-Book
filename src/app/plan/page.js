...
const FIXED_BUDGET = 51450 // ✅ Your fixed total budget
...
export default function MyPlan() {
  ...
  useEffect(() => { fetchRate().then(() => fetchData()); fetchHistory() }, [])
  ...
  async function handleAdd() {
    ...
    if (paying > inr) {
      setFormError('Amount Spent ₹ cannot exceed Total Amount ₹.')
      return
    }
  ...
  // ✅ FIXED BUDGET — totalINR only used for internal card calculations
  const totalINR     = rows.reduce((s, r) => s + Number(r.amount_rupee  || 0), 0)
  ...
  // ✅ All summary/progress calculations use FIXED_BUDGET
  ...
  const getActionIcon  = (a) => a === 'ADD' ? '➕' : a === 'EDIT' ? '✏️' : a === 'DELETE' ? '🗑️' : a === 'PAID' ? '✅' : '📝'
  const getActionColor = (a) => a === 'ADD' ? '#4dff91' : a === 'EDIT' ? '#6ab4ff' : a === 'DELETE' ? '#ff6b6b' : a === 'PAID' ? '#f7c26a' : '#ffa94d'

  // ✅ Auto-fill uses remaining balance from FIXED_BUDGET
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        @media(max-width:600px){.summary-grid{grid-template-columns:1fr}.root{padding:20px 14px}.cards-grid{grid-template-columns:1fr}}
      `}</style>
      ...
      <div className="root">
        ...
        {/* SUMMARY STATS — Total Amount always shows FIXED_BUDGET */}
        ...
        {/* PROGRESS BAR — based on FIXED_BUDGET */}
                ...
                <div key={item.id} className={`plan-card ${item.paid ? 'is-paid' : ''}`}>
                  ...
                  <div className="card-amounts">
                    ...
                    <div className="ca-block" style={{ gridColumn: '1 / -1' }}>
                      ...
                      <div className={`ca-val ${bal === 0 ? 'bal-zero' : bal > 0 ? 'bal-pos' : 'bal-neg'}`}>
                        {fmt(bal)}
                      </div>
                    </div>
                  </div>
