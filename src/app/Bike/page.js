...
const EMPTY_FORM = { month: '', amount_due: '', amount_paid: '', status: 'pending' }
...
export default function Home() {
  ...
  async function fetchData() {
    ...
    const { data, error } = await supabase
      .from('bike')
      .select('id, month, amount_due, amount_paid, status')
  ...
  async function logHistory(action, record_id, details) {
    await supabase.from('Bike_History').insert([{ action, record_id: String(record_id), details }])
    fetchHistory()
  }

  async function handleAdd() {
    setFormError(null)
    if (!form.month || !form.amount_due) { setFormError('Month and Amount Due are required.'); return }
    setSubmitting(true)
    const { data, error } = await supabase.from('bike').insert([{
      month: form.month,
    ...
    if (data?.[0]) {
      await logHistory('ADD', data[0].id, `Added record for ${form.month} — Due ₹${Number(form.amount_due).toLocaleString()}, Paid ₹${Number(form.amount_paid || 0).toLocaleString()}, Status: ${form.status}`)
    }
    setForm(EMPTY_FORM)
    setShowForm(false)
    fetchData()
  ...
  function startEdit(item) {
    ...
    setEditRow({ month: item.month, amount_due: item.amount_due, amount_paid: item.amount_paid, status: item.status })
  }

  async function saveEdit(id) {
    setSaving(true)
    const original = bike.find(r => r.id === id)
    const changes = []
    if (original.month !== editRow.month) changes.push(`Month: "${original.month}" → "${editRow.month}"`)
    if (Number(original.amount_due) !== Number(editRow.amount_due)) changes.push(`Amount Due: ₹${Number(original.amount_due).toLocaleString()} → ₹${Number(editRow.amount_due).toLocaleString()}`)
    if (Number(original.amount_paid) !== Number(editRow.amount_paid)) changes.push(`Amount Paid: ₹${Number(original.amount_paid).toLocaleString()} → ₹${Number(editRow.amount_paid).toLocaleString()}`)
    if (original.status !== editRow.status) changes.push(`Status: "${original.status}" → "${editRow.status}"`)

    const { error } = await supabase.from('bike').update({
      month: editRow.month,
      amount_due: Number(editRow.amount_due),
      amount_paid: Number(editRow.amount_paid),
  ...
  async function deleteRow(id) {
    ...
    const record = bike.find(r => r.id === id)
    await supabase.from('bike').delete().eq('id', id)
    await logHistory('DELETE', id, `Deleted record for ${record?.month || 'unknown'} — Due ₹${Number(record?.amount_due || 0).toLocaleString()}, Status: ${record?.status || '—'}`)
    fetchData()
  }

  const totalPaid = bike.reduce((s, r) => s + Number(r.amount_paid || 0), 0)
  const paidMonths = bike.filter(r => r.status === 'paid').length
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .stat-card.yellow::before { background: linear-gradient(90deg, #ffff4d, #ffd94d); }
        ...
        .stat-card.yellow .stat-value { color: #d9c91d; }
        ...
        .section-icon { width: 36px; height: 36px; border-radius: 10px; background: #1a1a0a; display: flex; align-items: center; justify-content: center; font-size: 1.1rem; }
        ...
        .data-table th { text-align: left; padding: 10px 12px; font-size: 0.7rem; text-transform: uppercase; letter-spacing: 1.5px; color: #444; border-bottom: 1px solid #1e1e2e; }
        .data-table td { padding: 10px 12px; font-size: 0.875rem; border-bottom: 1px solid #161622; font-family: 'JetBrains Mono', monospace; vertical-align: middle; }
      ...
      <div className="summary-grid">
        <div className="stat-card red">
          ...
          <div className="stat-value">₹62,190</div>
        </div>
        ...
        <div className="stat-card orange">
          ...
          <div className="stat-value">₹{(62190 - totalPaid).toLocaleString()}</div>
        </div>
      </div>
      ...
      <div className="summary-grid">
        <div className="stat-card" style={{ position:'relative' }}>
          <div style={{ position:'absolute', top:0, left:0, right:0, height:3, background:'linear-gradient(90deg, #edff4d, #c8ffa8)', borderRadius:'16px 16px 0 0' }} />
          <div className="stat-label">Total Months</div>
          <div className="stat-value" style={{ color:'#cbff6a' }}>18</div>
        </div>
        <div className="stat-card" style={{ position:'relative' }}>
          <div style={{ position:'absolute', top:0, left:0, right:0, height:3, background:'linear-gradient(90deg, #08b940, #8cf0a0)', borderRadius:'16px 16px 0 0' }} />
          <div className="stat-label">Months Paid</div>
          <div className="stat-value" style={{ color:'#4be474' }}>{bike.length}</div>
        </div>
        <div className="stat-card" style={{ position:'relative' }}>
          <div style={{ position:'absolute', top:0, left:0, right:0, height:3, background:'linear-gradient(90deg, #ea4dff, #ff97ee)', borderRadius:'16px 16px 0 0' }} />
          <div className="stat-label">Months Pending</div>
          <div className="stat-value" style={{ color:'#ffaaff' }}>{18 - bike.length}</div>
        </div>
      </div>
      ...
      <div className="section">
        <div className="section-header">
          ...
          <span className="section-badge">{bike.length} records</span>
        </div>
          ...
          <table className="data-table">
            ...
            <tbody>
              {bike.map((item) => {
                ...
                const due  = Number(isEditing ? editRow.amount_due  : item.amount_due  || 0)
                const paid = Number(isEditing ? editRow.amount_paid : item.amount_paid || 0)
                const rem  = due - paid
                return (
                  <tr key={item.id} className={isEditing ? 'editing' : ''}>
                    ...
                    <td>{isEditing ? <input className="inline-input" type="number" value={editRow.amount_due} onChange={e => setEditRow(p => ({ ...p, amount_due: e.target.value }))} /> : <span className="amount-negative">₹{Number(item.amount_due||0).toLocaleString()}</span>}</td>
                    <td>{isEditing ? <input className="inline-input" type="number" value={editRow.amount_paid} onChange={e => setEditRow(p => ({ ...p, amount_paid: e.target.value }))} /> : <span className="amount-positive">₹{Number(item.amount_paid||0).toLocaleString()}</span>}</td>
                    <td><span className={rem > 0 ? 'amount-warning' : 'amount-positive'}>₹{rem.toLocaleString()}</span></td>
                    <td>
                      {isEditing
                        ? <select className="inline-select" value={editRow.status} onChange={e => setEditRow(p => ({ ...p, status: e.target.value }))}>
                            <option value="pending">pending</option>
                            <option value="paid">paid</option>
                          </select>
                        : <span className={`badge badge-${item.status === 'paid' ? 'paid' : 'pending'}`}>{item.status || 'pending'}</span>}
                    </td>
                    <td>
          ...
          <div className="modal">
            ...
            <div className="form-row">
              <div className="form-group">
                ...
                <input className="form-input" type="number" placeholder="0" value={form.amount_due} onChange={e => setForm(p => ({ ...p, amount_due: e.target.value }))} />
              </div>
              <div className="form-group">
                <label>Amount Paid (₹)</label>
                <input className="form-input" type="number" placeholder="0" value={form.amount_paid} onChange={e => setForm(p => ({ ...p, amount_paid: e.target.value }))} />
              </div>
            </div>
            <div className="form-group">
              <label>Status</label>
